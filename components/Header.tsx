  ...
  return (
    <header className={`fixed z-10 top-0 left-0 right-0 flex justify-between items-center bg-white/30 backdrop-blur-xl backdrop-filter py-4 px-2 border border-(--secondary-color)`}>
        ...
        <Link href="/">
          ...
          <nav className={`sm:space-x-2 py-2 pl-2 right-0 space-y-2 flex-col sm:flex-row font-sans absolute sm:relative top-full left-0 w-full z-50 overflow-hidden transition-all duration-300 ease-in-out ${openMenu ? 'flex opacity-100 translate-y-0 bg-background border border-(--secondary-color)' : 'hidden -translate-y-[200px]'}`}>
