  ...
  return (
    <header className={`fixed px-[32px] z-10 top-0 left-0 right-0 flex justify-between items-center bg-white/30 backdrop-blur-xl backdrop-filter py-4 px-2`}>
        ...
        <Link href="/">
          ...
          <nav ref={dropdownRef} className={`sm:space-x-4 md:max-w-[1200px] py-2 pl-[32px] sm:pl-2 right-0 space-y-2 flex-col sm:flex-row font-sans absolute sm:relative sm:hidden md:flex top-full left-0 w-full z-50 overflow-hidden transition-all duration-300 ease-in-out ${openMenu ? 'flex opacity-100 translate-y-0 bg-background px-[32px]' : 'hidden sm:flex -translate-y-50 justify-center items-center sm:translate-y-0'}`}>
              ...
              <Link onClick={() => setCloseMenu(false)} className="flex gap-2 text-(--text) py-2 sm:border-none hover:text-(--secondary-color)" href="/#contact"><Headset/>Contact Us</Link>
              
              {
                sessionId ? <SignOutButton><button onClick={() => setCloseMenu(false)} className="bg-(--primary-color) px-4 py-2 text-white rounded-md sm:px-5 sm:py-2.5">Sign Out</button></SignOutButton> : <Link href="/login"><SignInButton><button onClick={() => setCloseMenu(false)} className="bg-(--primary-color) px-4 py-2 text-white rounded-md sm:px-5 sm:py-2.5 w-full">Sign In</button></SignInButton></Link>
              }
          </nav>
